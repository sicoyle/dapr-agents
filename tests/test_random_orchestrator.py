...
async def test_random_orchestrator_initialization(orchestrator_config):
    ...
    with patch('dapr_agents.workflow.orchestrators.random.OrchestratorWorkflowBase.model_post_init') as mock_init:
...
async def test_process_input(orchestrator_config):
    ...
    with patch('dapr_agents.workflow.orchestrators.random.OrchestratorWorkflowBase.model_post_init'):
        orchestrator = RandomOrchestrator(**orchestrator_config)
        task = "test task"
...
async def test_select_random_speaker(orchestrator_config):
    ...
    with patch('dapr_agents.workflow.orchestrators.random.OrchestratorWorkflowBase.model_post_init'), \
         patch.object(RandomOrchestrator, 'get_agents_metadata', return_value={
            "agent1": {"name": "agent1"},
            "agent2": {"name": "agent2"}
         }):
        orchestrator = RandomOrchestrator(**orchestrator_config)
