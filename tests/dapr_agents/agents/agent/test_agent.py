...
class TestAgent:
    ...
    async def test_run_with_cancellation(self, basic_agent):
        ...
        with patch.object(basic_agent, '_run_agent', side_effect=asyncio.CancelledError):
            result = await basic_agent.run("test input")
            assert result is None
    ...
    async def test_run_with_exception(self, basic_agent):
        ...
        with patch.object(basic_agent, '_run_agent', side_effect=Exception("Test error")):
    ...
    async def test_run_agent_with_tool_calls(self, agent_with_tools):
        ...
        final_response.get_message.return_value = AssistantMessage(content="Final answer")
    ...
    async def test_process_response_failure(self, agent_with_tools):
        ...
        with pytest.raises(AgentError, match=f"Error executing tool '{error_tool.name}': .*Tool failed"):
            await agent_with_tools.process_response([tool_call])
    ...
    async def test_process_iterations_with_llm_error(self, basic_agent):
        ...
        with pytest.raises(AgentError, match="Failed during chat generation: LLM error"):
    ...
    async def test_run_tool_failure(self, agent_with_tools):
        ...
        agent_with_tools.tools = [error_tool]
        agent_with_tools._tool_executor = agent_with_tools._tool_executor.__class__(tools=[error_tool])
        with pytest.raises(AgentError, match=f"Failed to run tool '{error_tool.name}': .*Tool failed"):
