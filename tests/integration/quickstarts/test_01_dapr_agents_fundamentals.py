...
class TestHelloWorldQuickstart:
    ...
    def test_agent_with_llm(self, dapr_runtime):  # noqa: ARG002
        """Test simple agent with only LLM (01_agent_llm_tools.py).
        ...
        """
        script_path = self.quickstart_dir / "01_agent_llm_tools.py"
    ...
    def test_agent_with_mcp(self, dapr_runtime):  # noqa: ARG002
        """Test agent with MCP tools example (02_agent_mcp_tools.py)."""
        script_path = self.quickstart_dir / "02_agent_mcp_tools.py"
        result = run_quickstart_script(
            script_path,
            cwd=self.quickstart_dir,
            env=self.env,
    ...
    def test_agent_with_memory(self, dapr_runtime):  # noqa: ARG002
        """Test agent with memory example (03_agent_memory.py).

        Note: dapr_runtime parameter ensures Dapr is initialized before this test runs.
        The fixture is needed for setup, even though we don't use the value directly.
        """
        script_path = self.quickstart_dir / "03_agent_memory.py"
        result = run_quickstart_script(
            script_path,
            cwd=self.quickstart_dir,
            env=self.env,
    ...
    def test_durable_agent_serve(self, dapr_runtime):  # noqa: ARG002
        """Test durable agent serve example (04_durable_agent_http.py).

        Note: dapr_runtime parameter ensures Dapr is initialized before this test runs.
        The fixture is needed for setup, even though we don't use the value directly.
        """
        script_path = self.quickstart_dir / "04_durable_agent_http.py"
        result = run_quickstart_script(
            script_path,
            cwd=self.quickstart_dir,
            env=self.env,
    ...
    def test_durable_agent_subscribe(self, dapr_runtime):  # noqa: ARG002
        """Test durable agent subscription example (05_durable_agent_pubsub.py).

        Note: dapr_runtime parameter ensures Dapr is initialized before this test runs.
        The fixture is needed for setup, even though we don't use the value directly.
        """
        script_path = self.quickstart_dir / "05_durable_agent_pubsub.py"
        result = run_quickstart_script(
            script_path,
            cwd=self.quickstart_dir,
            env=self.env,
    ...
    def test_durable_agent_trace(self, dapr_runtime):  # noqa: ARG002
        """Test durable agent trace example (08_durable_agent_tracing.py)."""
        script_path = self.quickstart_dir / "08_durable_agent_tracing.py"
        result = run_quickstart_script(
            script_path,
            cwd=self.quickstart_dir,
            env=self.env,
    ...
    def test_workflow_with_llms(self, dapr_runtime):  # noqa: ARG002
        """Test workflow with LLM activities example (06_workflow_llm.py).

        Note: dapr_runtime parameter ensures Dapr is initialized before this test runs.
        The fixture is needed for setup, even though we don't use the value directly.
        """
        script_path = self.quickstart_dir / "06_workflow_llm.py"
        result = run_quickstart_script(
            script_path,
            cwd=self.quickstart_dir,
            env=self.env,
    ...
    def test_workflow_with_agents(self, dapr_runtime):  # noqa: ARG002
        """Test workflow with agent activities example (07_workflow_agents.py).

        Note: dapr_runtime parameter ensures Dapr is initialized before this test runs.
        The fixture is needed for setup, even though we don't use the value directly.
        """
        script_path = self.quickstart_dir / "07_workflow_agents.py"
        result = run_quickstart_script(
            script_path,
            cwd=self.quickstart_dir,
            env=self.env,
