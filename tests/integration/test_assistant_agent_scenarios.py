...
class TestAssistantAgentScenarios:
    ...
    def assistant_agent_with_tools(self):
        ...
        return AssistantAgent(
            ...
            instructions=["Use weather tool for weather questions", "Use calculator for math"],
    ...
    def assistant_agent_with_memory(self):
        ...
        return AssistantAgent(
            ...
            instructions=["Remember what users tell you", "Reference previous conversations"],
            tools=[get_weather],
            llm=DaprChatClient(),
    ...
    @pytest.mark.skipif(
        not pytest.dapr_available, 
        reason="Dapr runtime not available"
    )
    ...
    async def test_assistant_agent_basic_workflow_execution(self, assistant_agent_basic):
    ...
    async def test_assistant_agent_tool_calling_workflow(self, assistant_agent_with_tools):
    ...
    async def test_assistant_agent_multi_iteration_workflow(self, assistant_agent_with_tools):
    ...
    async def test_assistant_agent_memory_persistence(self, assistant_agent_with_memory):
        ...
        try:
            ...
            weather_calls = [tool for tool in tool_history_2 if tool["function_name"] == "get_weather"]
    ...
    async def test_assistant_agent_error_handling(self, assistant_agent_with_tools):
        ...
        try:
            ...
            if tool_history:
                calc_tool = next((tool for tool in tool_history if tool["function_name"] == "calculate"), None)
                if calc_tool:
