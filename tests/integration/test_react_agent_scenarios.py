...
class TestReActAgentScenarios:
    ...
    async def test_react_agent_multi_tool_workflow(self, dapr_runtime, workflow_tools):
        ...
        assert any(word in response_lower for word in ["weather", "cloudy", "activity", "activities"])

    @pytest.mark.asyncio 
    async def test_react_agent_complex_reasoning_chain(self, dapr_runtime, simple_tools):
        ...
        assert any(word in response_lower for word in ["new york", "weather", "rainy"])  # Weather
...
class TestReActAgentProviders:
    ...
    async def test_react_agent_with_openai_provider(self, dapr_runtime, basic_tools, test_environment):
    ...
    @pytest.mark.asyncio
    @pytest.mark.requires_api_key
    async def test_react_agent_with_anthropic_provider(self, dapr_runtime, basic_tools, test_environment):
...
class TestReActAgentIntegration:
    ...
    async def test_react_agent_full_workflow_integration(self, dapr_runtime):
        ...
        response_lower = response.lower()
        assert any(word in response_lower for word in ["database", "search", "customer"])
