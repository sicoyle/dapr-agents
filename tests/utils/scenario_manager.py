...
class ScenarioManager:
    ...
    def detect_scenario(self) -> DevelopmentScenario:
        ...
        if all(development_repos.values()):
            ...
            not development_repos[repo] for repo in ["dapr", "python_sdk", "components_contrib"]
        ):
    ...
    def _detect_development_repos(self) -> Dict[str, bool]:
        ...
        return {
            ...
            "components_contrib": self._is_local_development(self.components_contrib_root),
    ...
    def validate_scenario_requirements(self, scenario: DevelopmentScenario) -> Dict[str, bool]:
    ...
    def _check_dapr_runtime(self) -> bool:
        ...
        try:
            result = subprocess.run(["dapr", "--version"], capture_output=True, timeout=5)
    ...
    def _check_local_sdk_build(self) -> bool:
        ...
        return (self.python_sdk_root / "setup.py").exists() or \
               (self.python_sdk_root / "pyproject.toml").exists()
    ...
    def get_scenario_description(self, scenario: DevelopmentScenario) -> str:
        ...
        descriptions = {
            DevelopmentScenario.LOCAL_FULL: 
                "Full local development - all repositories (dapr, python-sdk, components-contrib, dapr-agents) in development",
            DevelopmentScenario.LOCAL_PARTIAL:
                "Partial local development - python-sdk and dapr-agents in development, others released",
            DevelopmentScenario.AGENT_ONLY:
                "Agent-only development - only dapr-agents in development, all dependencies released",
            DevelopmentScenario.PRODUCTION:
                "Production-like testing - all dependencies use released versions"
        }
