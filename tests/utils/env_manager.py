...
class EnvManager:
    ...
    def _setup_local_full_scenario(self, setup_result: Dict):
        ...
        for repo in required_repos:
            if not dev_repos.get(repo, False):
                setup_result["errors"].append(f"{repo} repository not found or not in development")
                setup_result["requirements_met"] = False
        
        # Check for build requirements
        build_requirements = self.check_build_requirements(DevelopmentScenario.LOCAL_FULL)
    ...
    def check_build_requirements(self, scenario: DevelopmentScenario) -> Dict[str, bool]:
        """Check if build requirements are met for scenario."""
        requirements = {}
        
        if scenario in [DevelopmentScenario.LOCAL_FULL, DevelopmentScenario.LOCAL_PARTIAL]:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--create-template", action="store_true", help="Create test env template")
