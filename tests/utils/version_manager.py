...
class VersionManager:
    ...
    def check_compatibility(self, versions: Dict[str, str]) -> Dict[str, bool]:
        ...
        compatibility["python_supported"] = python_major_minor in ["3.8", "3.9", "3.10", "3.11", "3.12"]
        
        # Dapr runtime compatibility
        dapr_version = versions.get("dapr")
        if dapr_version:
            dapr_major_minor = ".".join(dapr_version.split(".")[:2])
            compatibility["dapr_supported"] = self._is_dapr_version_supported(dapr_major_minor)
        else:
    ...
    def _check_dapr_sdk_compatibility(self, dapr_version: str, sdk_version: str) -> bool:
