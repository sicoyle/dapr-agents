...
def install_dapr_cli() -> bool:
    ...
    try:
        if system == "darwin":  # macOS
            cmd = ["curl", "-fsSL", "https://raw.githubusercontent.com/dapr/cli/master/install/install.sh"]
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            subprocess.run(["bash"], input=result.stdout, text=True, check=True)
        elif system == "linux":
            cmd = ["wget", "-q", "https://raw.githubusercontent.com/dapr/cli/master/install/install.sh", "-O", "-"]
            result = subprocess.run(cmd, capture_output=True, text=True, check=True)
            subprocess.run(["bash"], input=result.stdout, text=True, check=True)
        else:
            print_error(f"Unsupported OS: {system}")
            print_error("Please install Dapr CLI manually: https://docs.dapr.io/getting-started/install-dapr-cli/")
            return False
...
def main():
    ...
    print("  cd tests && python -m pytest integration/test_basic_integration.py::test_echo_conversation -v")
    print("\n  # All chat scenarios")
    print("  cd tests && python -m pytest integration/test_chat_scenarios.py -v -s")
    print("\n  # React agent tests")  
    print("  cd tests && python -m pytest integration/test_react_agent_scenarios.py -v -s")
