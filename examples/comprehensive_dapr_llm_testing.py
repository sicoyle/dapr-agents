    ...
    try:
        with DaprClient() as client:
            inputs = [ConversationInput(
                content="Hello from the echo component test!",
                role="user"
            )]
...
def test_anthropic_component():
    ...
    try:
        with DaprClient() as client:
            ...
            print("📤 Streaming: What's the difference between AI and machine learning?")
            print("📥 Streamed response: ", end="", flush=True)
...
def show_component_configurations():
    ...
    for yaml_file in components_dir.glob("*-conversation.yaml"):
        ...
        try:
            ...
            lines = content.split('\n')
            name = next((line.split(':')[1].strip() for line in lines if 'name:' in line), 'unknown')
            comp_type = next((line.split(':')[1].strip() for line in lines if 'type:' in line), 'unknown')
            print(f"   Name: {name}")
...
def main():
    ...
    parser.add_argument(
        "--show-config", 
        action="store_true",
        help="Show component configurations"
    )
