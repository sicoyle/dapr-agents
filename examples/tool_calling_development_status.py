...
def demonstrate_current_state():
    ...
    print(f"   Parameters: {list(openai_format['function']['parameters']['properties'].keys())}")
...
def demonstrate_dapr_agents_integration():
    ...
    try:
        ...
        messages = [{"role": "user", "content": "What's the weather in San Francisco? Use the weather tool."}]
        tools = [get_weather]
...
if __name__ == "__main__":
    print(f"🧪 Working Tool Calling Example - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    print("🎯 SUMMARY")
    print("=" * 50)
    print("✅ Foundation is solid - tool definitions, data structures, processing all work")
