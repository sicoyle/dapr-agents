...
def calculate_math(expression: str) -> str:
    ...
    try:
        ...
        if all(c in allowed_chars for c in expression):
            ...
            return "Invalid expression: {}. Only basic math operations allowed.".format(
                expression
            )
...
def create_agent() -> DaprWorkflowAgent:
    ...
    model = OpenAIServerModel(
        model_id="gpt-3.5-turbo", api_key="test-key-replace-with-real"
    )
...
def test_basic_functionality():
    ...
    try:
        ...
        print(
            "   - Has workflow runtime: {}".format(agent.workflow_runtime is not None)
        )
...
def test_context_manager(agent: DaprWorkflowAgent):
    ...
    try:
        with agent as ctx_agent:
            ...
            print(
                "   - Context agent workflow name: {}".format(ctx_agent.workflow_name)
            )
            print(
                "   - Runtime is active: {}".format(
                    ctx_agent.workflow_runtime is not None
                )
            )
...
def test_workflow_monitoring(agent: DaprWorkflowAgent):
    ...
    try:
        ...
        print(
            "   - get_workflow_status: {}".format(hasattr(agent, "get_workflow_status"))
        )
        print(
            "   - terminate_workflow: {}".format(hasattr(agent, "terminate_workflow"))
        )
