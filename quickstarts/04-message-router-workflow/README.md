...
├─ handlers.py                 # (exists but not used in current pattern)
└─ message_client.py           # publishes a test message to the topic
```

**Note:** In the current implementation, the `@message_router` decorator is applied directly to the `blog_workflow` function in `workflow.py`, making it both the workflow and the message handler.
