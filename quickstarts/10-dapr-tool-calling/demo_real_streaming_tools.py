...
def test_streaming_tool_calling(provider: str, test_cases: List[dict]):
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(
            f"{Fore.MAGENTA}🤖 Streaming Response:{Style.RESET_ALL} ",
            end="",
            flush=True,
        )
        ...
        try:
                        ...
                        for output in chunk.complete.outputs:
                            if hasattr(output, "tool_calls") and output.tool_calls:
                                ...
                                for i, tool_call in enumerate(output.tool_calls):
                                    print(
                                        f"  {Fore.MAGENTA}Tool {i+1}:{Style.RESET_ALL}"
                                    )
            ...
            if tool_calls_detected:
                print(
                    f"{Fore.YELLOW}🔧 Tool calls detected in response{Style.RESET_ALL}"
                )
            else:
...
def main():
    ...
    if not providers:
        print(
            f"{Fore.RED}❌ No API keys found! Please set OPENAI_API_KEY or ANTHROPIC_API_KEY{Style.RESET_ALL}"
        )
        return

    print(
        f"{Fore.GREEN}🔑 Available providers: {', '.join(providers)}{Style.RESET_ALL}"
    )
    ...
    print(
        f"{Fore.GREEN}✅ All streaming + tool calling tests completed!{Style.RESET_ALL}"
    )
