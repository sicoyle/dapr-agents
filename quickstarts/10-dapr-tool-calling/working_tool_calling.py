...
def main():
    ...
    print(f"{Fore.YELLOW}This example demonstrates intelligent tool selection using available APIs{Style.RESET_ALL}\n")
    ...
    try:
        llm = DaprChatClient()
        print(f"{Fore.GREEN}✅ DaprChatClient initialized successfully{Style.RESET_ALL}")
    except Exception as e:
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        print(f"{Fore.CYAN}📋 Test {i}/{len(test_scenarios)}: {scenario['name']}{Style.RESET_ALL}")
        print(f"{Fore.GREEN}❓ Query:{Style.RESET_ALL} {scenario['prompt']}")
        print(f"{Fore.BLUE}🔧 Expected Tool:{Style.RESET_ALL} {scenario['expected_tool']}")
        ...
        try:
            ...
            if hasattr(response, 'choices') and response.choices:
                ...
                tool_calls_detected = "function" in content.lower() or "tool" in content.lower()
                
                print(f"\n{Fore.MAGENTA}✅ Scenario completed in {end_time - start_time:.2f}s{Style.RESET_ALL}")
                if tool_calls_detected:
                    print(f"{Fore.GREEN}🎯 Tool calling detected in response{Style.RESET_ALL}")
                else:
                    print(f"{Fore.YELLOW}⚠️  No obvious tool calling detected{Style.RESET_ALL}")
            else:
                print(f"{Fore.RED}❌ Unexpected response format: {response}{Style.RESET_ALL}")
    ...
    try:
        ...
        if hasattr(simple_response, 'choices') and simple_response.choices:
            ...
            print(f"{Fore.RED}❌ Unexpected response format: {simple_response}{Style.RESET_ALL}")
            
    except Exception as e:
        print(f"{Fore.RED}❌ Error in simple conversation: {e}{Style.RESET_ALL}")
    
    print(f"\n{Fore.CYAN}🎉 Tool Calling Test Complete!{Style.RESET_ALL}")
    print(f"{Fore.MAGENTA}📝 Note: Full streaming + tool calling will be available when converse_stream_alpha1 is implemented{Style.RESET_ALL}")
