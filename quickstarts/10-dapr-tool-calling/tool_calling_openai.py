...
def calculate_compound_interest(
    principal: float, rate: float, time: int, compound_frequency: int = 12
) -> dict:
    """
...
def main():
    ...
    print(
        f"{Fore.YELLOW}This example demonstrates intelligent tool selection and execution{Style.RESET_ALL}\n"
    )
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        try:
            ...
            response = llm.generate(prompt=scenario["prompt"], tools=tools)
            ...
            print(
                f"{Fore.YELLOW}⏱️  Response time: {end_time - start_time:.2f}s{Style.RESET_ALL}"
            )
    ...
    print(
        f"{Fore.YELLOW}Type your questions, and the AI will use tools as needed. Type 'quit' to exit.{Style.RESET_ALL}\n"
    )
