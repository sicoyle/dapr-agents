...
def check_provider_requirements(provider: str):
    ...
    if provider in requirements:
        ...
        if not api_key:
            print(f"⚠️  Warning: {requirements[provider]} not found in environment variables or .env file")
...
def run_simple_example(provider: str = "echo-tools", streaming: bool = False, use_class_tools: bool = False):
