    ...
    def load_config_with_global(self, config_file: str) -> Dict[str, Any]:
        ...
        if "global_config" in config:
            global_config_path = os.path.join(
                os.path.dirname(config_file), config["global_config"]
            )
            ...
            if "agent_config" in config and config["agent_config"] in global_config.get(
                "agent", {}
            ):
    ...
    def apply_to_object(self, obj: Any, config: Dict[str, Any], section: str):
        ...
        for key, value in section_config.items():
            if hasattr(obj, key):
                ...
                logger.warning(
                    f"Unknown configuration key '{key}' for section '{section}'"
                )
