...
def generate_get_openapi_definition(tool_vector_store: VectorToolStore):
    ...
    def get_openapi_definition(user_input: str):
        ...
        """
        similatiry_result = tool_vector_store.get_similar_tools(
            query_texts=[user_input], k=5
        )
...
def generate_api_call_executor(
    spec_parser: OpenAPISpecParser, auth_header: Dict = None
):
    base_url = spec_parser.spec.servers[0].url

    class OpenAPIExecutorInput(BaseModel):
        path_template: str = Field(
            description="Template of the API path that may include placeholders."
        )
        method: str = Field(
            description="The HTTP method to be used for the API call (e.g., 'GET', 'POST')."
        )
        path_params: Dict[str, Any] = Field(
            default={},
            description="Path parameters to be replaced in the path template.",
        )
        data: Dict[str, Any] = Field(
            default={},
            description="Data to be sent in the body of the request, applicable for POST, PUT methods.",
        )
        headers: Optional[Dict[str, Any]] = Field(
            default=None, description="HTTP headers to send with the request."
        )
        params: Optional[Dict[str, Any]] = Field(
            default=None, description="Query parameters to append to the URL."
        )
