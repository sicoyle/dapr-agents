...
def add_signal_handlers_cross_platform(loop: asyncio.AbstractEventLoop, handler_func, signals=(signal.SIGINT, signal.SIGTERM)):
    """
        ...
        for sig in signals:
            try:
                loop.add_signal_handler(
                    sig,
                    lambda s=sig: asyncio.create_task(handler_func(s))
                )
